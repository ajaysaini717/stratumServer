...
type Storage interface {
	...
	AddLogRotation(ctx context.Context, cid string) error
}
...
func (s *PostgresStorage) AddLogRotation(ctx context.Context, cid string) error {
	query := `INSERT INTO log_rotations (cid) VALUES ($1)`
	_, err := s.pool.Exec(ctx, query, cid)
	if err != nil {
		return fmt.Errorf("AddLogRotation error: %w", err)
	}
	return nil
}
